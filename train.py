...
def train(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            # Update best mAP
            print('fadsfsaddsafdsadsadsafsdafasdfdsafasd\n\n\n\n\n\n',np.array(results).reshape(1, -1))
...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            # Update best mAP
            fi = fitness(np.array(results).reshape(1, -1))  # weighted combination of [P, R, mAP@.5, mAP@0.75, mAP@.5-.95]
            if fi > best_fitness:
                best_fitness = fi
            wandb_logger.end_epoch(best_result=best_fitness == fi)
    ...
    if rank in [-1, 0]:
        ...
        if opt.data.endswith('coco.yaml') and nc == 80:  # if COCO
            ...
            m = best if best.exists() else last
            results, maps, times = test.test(data_dict,
                                                 batch_size=batch_size * 2,
                                                 imgsz=imgsz_test,
                                                 model=attempt_load(m, device).half(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./models/WMNet.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='./data/multispectral/ir.yaml', help='data.yaml path')
