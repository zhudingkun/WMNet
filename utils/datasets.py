...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if img0.shape[0]!=self.imgsize2:  # if sizes are not equal
            img0 = cv2.resize(img0,(self.img_size, self.imgsize2),interpolation=cv2.INTER_AREA if r < 1 else cv2.INTER_LINEAR)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        #print("llllllllllllllla",self.label_files)
        #exit(0)
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        bi_rgb = np.floor(np.arange(n_rgb) / batch_size).astype(np.int64)  # batch index
        ...
        if self.rect:
            ...
            self.batch_shapes_rgb = np.ceil(np.array(shapes_rgb) * img_size / stride + pad).astype(np.int64) * stride
            ...
            self.batch_shapes_ir = np.ceil(np.array(shapes_ir) * img_size / stride + pad).astype(np.int64) * stride
...
def load_image_rgb_ir(self, index):
    ...
    if (img_rgb is None) and (img_ir is None):  # not cached
        ...
        if img_rgb.shape!=img_ir.shape:
            img_ir = cv2.resize(img_ir, (img_rgb.shape[1],img_rgb.shape[0])) #resize to the same as rgb shape
